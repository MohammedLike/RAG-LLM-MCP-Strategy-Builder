...
class NLParser:
    ...
    def parse(self, text: str) -> Optional[dict]:
        ...
        # Look for explicit symbols or names
        ...
        if "rsi" in text_lower:
            nums = [float(n) for n in re.findall(r"\b\d+\.?\d*\b", text) if 5 < float(n) < 95]
            entry_val = 30.0
            exit_val = 70.0
            ...
            if len(nums) >= 2:
                ...
                val = nums[0]
                if val <= 50: entry_val = val
                else: exit_val = val
                    
            op = "<"
            if any(w in text_lower for w in ["above", ">", "over"]):
                op = ">"
